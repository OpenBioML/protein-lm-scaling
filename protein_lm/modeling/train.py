...
from protein_lm.modeling.tokenizer.tokenizer import AptTokenizer
...
def train(**kwargs):
    ...
    if mode == 'command_line':
      ...
      parser.add_argument('--max_model_length', default=1024, type=int, help='maximum length of tokenized sequence')
      parser.add_argument('--sequence_length', default=1024, type=int, help='length of sequence in dataset ')
      ...
      parser.add_argument('--checkpoint_dir',default=dir_path, type=str, help='Path of APT model checkpoints directory')
      ...
      max_model_length = kwargs.get('max_model_length',1024)
    ...
    assert lr > 0 and wd > 0 and epochs > 0 and steps > 0
    ...
    if train_file_path:
      ...
      train_ds = train_ds.map(lambda e: set_input_ids(result =e,tokenizer= tokenizer, sequence_column_name = sequence_column_name , max_sequence_length = max_model_length), batched=True)
